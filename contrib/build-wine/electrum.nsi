...
;Variables
  ...
  !define PRODUCT_NAME "Electrum Quark"
  !define PRODUCT_WEB_SITE "https://github.com/bitcoinquark/electrum"
  !define PRODUCT_PUBLISHER "Bitcoin Quark Organization"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-quark-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "tmp\electrum\icons\electrum-quark.ico"
...
Section
  ...
  File /r "dist\electrum-quark\*.*"
  File "..\..\icons\electrum-quark.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-quark-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-quark-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-quark-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-quark-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-quark-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoinquark: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\bitcoinquark" "" "URL:bitcoinquark Protocol"
  WriteRegStr HKCU "Software\Classes\bitcoinquark" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bitcoinquark" "DefaultIcon" "$\"$INSTDIR\electrum-quark.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitcoinquark\shell\open\command" "" "$\"$INSTDIR\electrum-quark-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-quark.ico"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\bitcoinquark"
